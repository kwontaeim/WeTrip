...
public class tempUploadSample {
	...
	ServletContext sc;
	@Autowired
	TravelMainFileDao boardFiledao;

	@RequestMapping(value = "fileUpload", method = RequestMethod.POST, produces = "application/json")
	@ResponseBody
	public String upload(MultipartHttpServletRequest req, HttpServletResponse res, HttpSession session)
			throws IOException {
	...
	/*
	 * @RequestMapping(path="userProfileFileLoder") public Object
	 * profileFileLoder (HttpSession session) { try { HoneyMembers honeyMember =
	 * (HoneyMembers)session.getAttribute("member"); MemberFile memberFile = new
	 * MemberFile();
	 * memberFile.setFilename(hMembersService.getProfileFileName(honeyMember.
	 * getMemberNo())); return JsonResult.success(memberFile.getFilename()); }
